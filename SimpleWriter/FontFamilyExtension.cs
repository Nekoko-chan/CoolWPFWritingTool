...
namespace ComplexWriter
{
    public static class FlowDocumentExtensions
    {
        ...
        {
            var fontname =
                  fontFamily.Source.Substring(Math.Max(0,fontFamily.Source.LastIndexOf("/#", StringComparison.OrdinalIgnoreCase)));
        ...
        public static FlowDocument CopyDocument(this FlowDocument original)
        {
            var ret = new FlowDocument();
            foreach (var block in original.Blocks)
            {
                Block blk = (Block)XamlReader.Parse(XamlWriter.Save(block));
                if (block.BreakPageBefore)
                    blk.BreakPageBefore = true;
                ret.Blocks.Add(blk);
            }
            return ret;
        }
